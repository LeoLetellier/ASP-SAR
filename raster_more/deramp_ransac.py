...
Usage: deramp_ransac.py <infile> --outfile=<outfile> [--ramp=<ramp>] [--add-data=<add-data>] [--plot] [--band=<band>] [--chunk-size=<chunk-size>] [--overwrite] [--ovr-ratio=<ovr-ratio>] [--ndv=<ndv>] [--mask-percentile=<mask-percentile>] [--save-ramp] [--nosave-deramp] [-v | --verbose] [--mask=<mask>] [-h | --help] [--save-coeffs] [--cyclic-data]
...
def discard_percentile(data: np.ndarray, percentile: float) -> np.ndarray:
    ...
    if not 0 < percentile < 50:
        raise ValueError("Percentile {} should be between 0 and 50 both excluded".format(percentile))
...
class Ramp:
    ...
    def from_component_powers(
        ...
        ramp = Ramp()
        ramp.__across_track_orders = across_track_orders if across_track_orders is not None else []
        ramp.__along_track_orders = along_track_orders if along_track_orders is not None else []
        ramp.__cross_dimension_orders = cross_dimension_orders if cross_dimension_orders is not None else []
        ramp.__extern_data_orders = extern_data_orders if extern_data_orders is not None else []
        return ramp
    ...
    def __len__(self):
        ...
        if self.has_cyclic_extern_data:
            add += 1
        return add + len(self.__across_track_orders) + len(self.__along_track_orders) + len(self.__cross_dimension_orders) + len(self.__extern_data_orders)
    ...
    def __str__(self):
        ...
        if self.has_cyclic_extern_data:
            ...
            coeff += 1
            equation.append("{}.cos(D-{})".format(letter, letter2))
        else:
        ...
        if self.coefficients is None or (len(self) != len(self.coefficients) and not self.has_cyclic_extern_data):
            return equation
    ...
    def get_matrix(self, across: np.ndarray, along: np.ndarray, extern_data: None | np.ndarray = None, with_reference: bool = True) -> np.ndarray:
        ...
        if len(along) != data_nb:
            raise ValueError("Size mismatch: across ({}) vs along ({})".format(data_nb, len(along)))
        if extern_data is not None and len(extern_data) != data_nb:
            raise ValueError("Size mismatch: data_nb ({}) vs extern_data ({})".format(data_nb, len(extern_data)))
        if len(self.__extern_data_orders) > 0 and extern_data is None:
            raise ValueError("Ramp got external data component but no external data is provided")
    ...
    def solve(self, data: np.ndarray, extern_data: np.ndarray | None = None, mask_percentile: float = 0.5):
    ...
    def generate(self, dimensions: list[int], chunk: list[int] | None = None, extern_data: np.ndarray | None = None):
        ...
        if len(self.coefficients) != len(self) and not self.has_cyclic_extern_data:
            raise AttributeError("Number of coefficients ({}) in the ramp has change since last solving ({})".format(len(self), len(self.coefficients)))
        if chunk is None:
        ...
        G = self.get_matrix(across_values, along_values, extern_data=extern_data, with_reference=True)
...
def gdal_raster_size(path: str) -> list[int]:
    ...
    """
    ds = gdal.Open(path)
    logger.info("Raster {} has size {}".format(path, [ds.RasterXSize, ds.RasterYSize, ds.RasterCount]))
    return ds.RasterXSize, ds.RasterYSize, ds.RasterCount


def open_gdal(path: str, band: int = 1, chunk: list[int] = None) -> np.ndarray:
...
def open_gdal_reduced(path: str, band: int = 1, size: list[int] | None = None, resample_alg=gdal.GRIORA_Bilinear) -> np.ndarray:
    ...
    if ndv is not None and ndv != np.nan:
        data[data == ndv] = np.nan
    logger.info("Opened full raster {} with overview of size ({},{}) and resampling algorithm '{}'".format(path, size[0], size[1], resample_alg))
    return data
...
def write_gdal_chunk(path: str, data: np.ndarray, offset: list[int], band: int = 1, ndv: float | None = None):
...
def plot_ramp_result(data: np.ndarray, ramp: np.ndarray):
    ...
    vminmax = max(abs(np.nanpercentile(data - ramp, 2)), abs(np.nanpercentile(data - ramp, 98)))
...
def get_ramp(ramp_name: str, has_extern_data: bool = False, use_cyclic_data: bool = False) -> Ramp:
...
if __name__ == "__main__":
    ...
    target_size = [int(s) for s in arguments["--chunk-size"].split(",")] if arguments["--chunk-size"] is not None else [1024, 1024]
    if len(target_size) == 1:
        target_size = [target_size[0], target_size[0]]
    elif len(target_size) > 2:
        raise ValueError("chunk size have max 2 values but {} were provided".format(len(target_size)))
    overwrite = arguments["--overwrite"]
    ratio = float(arguments["--ovr-ratio"]) if arguments["--ovr-ratio"] is not None else 1e6
    add_ndv = arguments["--ndv"]
    mask_percentile = arguments["--mask-percentile"] if arguments["--mask-percentile"] is not None else 0.2
    ...
    ramp = get_ramp(ramp_name, has_extern_data=not add_data is None, use_cyclic_data=use_cyclic_data)
    ...
    if mask is not None:
        mask_reduced = open_gdal_reduced(mask, size=size)
        data_reduced = np.where(mask_reduced >= 1, data_reduced, np.nan)
    if add_data is not None:
        dem_reduced = open_gdal_reduced(add_data, size=size)
        ramp.solve(data_reduced, dem_reduced, mask_percentile)
    else:
        ramp.solve(data_reduced, None, mask_percentile)
    if save_coeffs:
        outfile_coeffs = outfile + "_coeffs_{}.txt".format(datetime.now().strftime("%y%m%d%H%M%S"))
    ...
    if do_plot:
        ...
        if add_data is not None:
            ramp_array = ramp.generate(size, extern_data=dem_reduced)
        else:
            ramp_array = ramp.generate(size)
    ...
    if do_save_deramp or do_save_ramp:
        ...
        if do_save_ramp:
            if os.path.isfile(outfile_ramp):
                if not overwrite:
                    raise FileExistsError("Raster already exists: {}. Use --overwrite".format(outfile_ramp))
            else:
        ...
        for i, c in enumerate(tqdm(chunks, unit="chunk")):
            ...
            if add_data is not None:
                add_data_chunk = open_gdal(add_data, chunk=c)
                ramp_array = ramp.generate(raster_size, chunk=c, extern_data=add_data_chunk)
            else:
