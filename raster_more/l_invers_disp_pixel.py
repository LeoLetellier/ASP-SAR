...
Usage: 
    invers_disp_pixel.py (<cols> <lines> | <region>) [--cube=<cube> --list_images=<list_images> --windowsize=<windowsize> \
...
--mask                  Crop (xmin,xmax,ymin,ymax) or mask raster
...
print("Revised Version: November 2025 Leo Letellier")
print()
print()
...
if __name__ == "__main__":
    ...
    for i, p in enumerate(pixels_date_data):
        print("Point [{}]".format(i + 1))
        ##> Solved an owned instance of the model for the time series of a given pixel
        time_model_p = time_model.copy().solve(p, rms)
        ##<
        print(time_model_p)
        position = [int(arguments["<cols>"].split(',')[i]), int(arguments["<lines>"].split(',')[i])]
        time_model_p.plot(dates.astype(str), p, band=last_band, position=position)
        plt.show()
